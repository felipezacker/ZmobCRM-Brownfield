...
- [x] AC1: Botão "Salvar Fila" no painel de filtros (aba Queue, após aplicar filtros)
- [x] AC2: Ao clicar, modal pede nome da fila (ex: "Leads frios 30 dias") e salva filtros atuais
- [x] AC3: Lista de filas salvas acessível via dropdown/botão "Minhas Filas" no header da aba Queue
- [x] AC4: Ao selecionar fila salva, filtros são aplicados automaticamente e resultados carregados
- [x] AC5: Opção de "Excluir" fila salva (com confirmação)
- [x] AC6: Diretor/admin pode marcar fila como "compartilhada" — visível para todos os corretores da org
- [x] AC7: Filas salvas persistem na tabela `prospecting_saved_queues` com RLS por organization_id
- [x] AC8: Botão "Exportar PDF" no dashboard de métricas (aba Metrics)
- [x] AC9: PDF contém: período selecionado, KPIs resumidos, gráfico de chamadas, ranking de corretores (se diretor/admin), insights gerados
- [x] AC10: PDF gerado client-side (sem dependência de servidor)
- [x] AC11: PDF inclui header com nome da organização e data de geração
- [x] AC12: Dark mode + responsivo
...
| 2026-03-06 | @po | Story closed — Status InReview → Done. Epic CP-2 completo (4/4 stories). |
