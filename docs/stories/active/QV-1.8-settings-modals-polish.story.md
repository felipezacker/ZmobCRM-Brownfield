...
- [ ] Task 1 (AC1): Investigar e corrigir input de custom field no modal completo do contato
  - [ ] 1.1: Abrir `features/contacts/components/ContactDetailModal.tsx` e verificar se ha componente que renderiza inputs para `custom_fields`
  - [ ] 1.2: Se o componente de input existir, identificar o problema (onSubmit, onChange ou state binding incorreto)
  - [ ] 1.3: Se o componente NAO existir, registrar como feature (escopo expandido) e avisar o @po antes de implementar
  - [ ] 1.4: Corrigir o wiring do form state e validar que o valor e persistido apos submit
- [ ] Task 2 (AC2): Investigar e corrigir save de produto em settings
  - [ ] 2.1: Abrir `features/settings/components/ProductsCatalogManager.tsx` e identificar o fluxo de save
  - [ ] 2.2: Verificar se ha optimistic update que reverte silenciosamente ou race condition na invalidacao de cache
  - [ ] 2.3: Corrigir o problema e garantir que a edicao persiste na primeira tentativa
- [ ] Task 3 (AC3): Adicionar botao de editar tag em settings
  - [ ] 3.1: Abrir `features/settings/components/TagsManager.tsx` e analisar o pattern de delete existente
  - [ ] 3.2: Replicar o pattern de acoes (semelhante ao custom fields) adicionando icone de lapis para edicao
  - [ ] 3.3: Implementar modal ou inline edit para alterar o nome da tag e persistir
- [ ] Task 4 (AC4): Aumentar z-index do toast container para aparecer acima de modais
  - [ ] 4.1: Em `context/ToastContext.tsx` linha 116, alterar `z-50` para `z-[calc(var(--z-modal)+1)]` (efetivo: z-301)
  - [ ] 4.2: Verificar que `--z-modal: 300` esta definido em `app/globals.css` (linha 79) — confirmar que a CSS variable e acessivel via Tailwind
  - [ ] 4.3: Testar com modal aberto: disparar toast e confirmar que aparece acima do overlay
- [ ] Task 5 (AC5): Implementar stack de modais para ESC
  - [ ] 5.1: Analisar como `lib/a11y/components/FocusTrap.tsx` captura o evento ESC
  - [ ] 5.2: Identificar se o bug ocorre porque ambos FocusTrap (pai e filho) recebem o evento ESC simultaneamente
  - [ ] 5.3: Corrigir o conflito — apenas o FocusTrap mais interno deve processar ESC (usar `stopPropagation` ou um registro global de stack de modais)
  - [ ] 5.4: Testar com 2+ modais empilhados (Modal base + ConfirmModal) e validar que ESC fecha apenas o filho
- [ ] Task 6: Garantir qualidade
  - [ ] 6.1: `npm run typecheck` passa sem erros
  - [ ] 6.2: `npm run lint` passa sem erros
  - [ ] 6.3: `npm test` passa sem regressoes
