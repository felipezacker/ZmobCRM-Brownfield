    ...
    <div className="space-y-3">
      {displayed.map((item, idx) => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="text-xs text-secondary-foreground dark:text-muted-foreground w-20 truncate text-right" title={item.label}>
            {item.label}
          </span>
          <div className="flex-1 h-6 bg-muted dark:bg-card rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${item.color || DEFAULT_COLORS[idx % DEFAULT_COLORS.length]}`}
              style={{ width: `${Math.max((item.value / max) * 100, 2)}%` }}
            />
          </div>
          <span className="text-xs font-medium text-secondary-foreground dark:text-muted-foreground w-10 text-right">
            {item.value}
          </span>
        </div>
      ))}
