        ...
        <div className="mt-2">
          {items && items.length > 0 ? (
            <div className="space-y-1.5 text-xs">
              {items.map((item, idx) => (
                <div key={item.id ?? idx} className="group flex items-center justify-between gap-2">
                  <span className="text-secondary-foreground dark:text-muted-foreground truncate">{item.name ?? 'Produto'}</span>
            ...
            <div className="flex items-center gap-2 py-2">
              <Package className="h-4 w-4 text-muted-foreground dark:text-secondary-foreground" />
              <span className="text-xs text-muted-foreground dark:text-muted-foreground">Nenhum produto adicionado</span>
            </div>
          )}
          {/* Add item form */}
          {onAddItem ? (
