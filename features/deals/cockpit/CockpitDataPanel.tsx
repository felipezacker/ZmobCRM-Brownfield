...
/** Labels para temperatura do lead (dark theme). */
const TEMPERATURE_CONFIG: Record<string, { label: string; color: string }> = {
  HOT: { label: 'Quente', color: 'bg-rose-500/15 text-rose-300 ring-rose-500/20' },
  WARM: { label: 'Morno', color: 'bg-amber-500/15 text-amber-300 ring-amber-500/20' },
  COLD: { label: 'Frio', color: 'bg-sky-500/15 text-sky-300 ring-sky-500/20' },
};

/** Labels para classificação do contato (dark theme). */
const CLASSIFICATION_CONFIG: Record<string, { label: string; color: string }> = {
  COMPRADOR: { label: 'Comprador', color: 'bg-blue-500/15 text-blue-300 ring-blue-500/20' },
  VENDEDOR: { label: 'Vendedor', color: 'bg-purple-500/15 text-purple-300 ring-purple-500/20' },
  LOCATARIO: { label: 'Locatário', color: 'bg-cyan-500/15 text-cyan-300 ring-cyan-500/20' },
  LOCADOR: { label: 'Locador', color: 'bg-teal-500/15 text-teal-300 ring-teal-500/20' },
  INVESTIDOR: { label: 'Investidor', color: 'bg-amber-500/15 text-amber-300 ring-amber-500/20' },
  PERMUTANTE: { label: 'Permutante', color: 'bg-orange-500/15 text-orange-300 ring-orange-500/20' },
};

/** Labels para prioridade. */
const PRIORITY_CONFIG: Record<string, { label: string; color: string }> = {
  high: { label: 'Alta', color: 'bg-rose-500/15 text-rose-300 ring-rose-500/20' },
  medium: { label: 'Média', color: 'bg-amber-500/15 text-amber-300 ring-amber-500/20' },
  low: { label: 'Baixa', color: 'bg-sky-500/15 text-sky-300 ring-sky-500/20' },
};
        ...
        <div>
          ...
          {tags && tags.length > 0 ? (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {tags.map((tag) => (
                <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] font-medium text-slate-300">
                  {tag}
                </span>
              ))}
            </div>
          ...
          {/* Contact Notes */}
          {contactNotes ? (
            <div className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/10 p-2 text-xs text-amber-200/90 whitespace-pre-wrap">
              {contactNotes}
            </div>
          ) : null}
        </div>
            ...
            <div className="space-y-1.5 text-xs">
              {preferences.purpose ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-slate-200">{PURPOSE_LABELS[preferences.purpose] ?? preferences.purpose}</span>
                </div>
              ) : null}
              {preferences.propertyTypes.length > 0 ? (
                <div className="flex items-center justify-between gap-2">
                  <span className="text-slate-500">Tipos</span>
                  <span className="text-slate-200">{preferences.propertyTypes.join(', ')}</span>
                </div>
              ) : null}
              ...
              {preferences.regions.length > 0 ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-slate-200 text-right">{preferences.regions.join(', ')}</span>
                </div>
              ) : null}
              {preferences.bedroomsMin !== null ? (
                <div className="flex items-center justify-between gap-2">
                  <span className="text-slate-500">Quartos mín.</span>
                  <span className="text-slate-200">{preferences.bedroomsMin}+</span>
                </div>
              ) : null}
              {preferences.urgency ? (
                <div className="flex items-center justify-between gap-2">
                  <span className="text-slate-500">Urgência</span>
                  <span className="text-slate-200">{URGENCY_LABELS[preferences.urgency] ?? preferences.urgency}</span>
                </div>
              ) : null}
            </div>
          ) : (
        ...
        <div className="rounded-xl border border-white/10 bg-white/2 p-3">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="font-semibold text-slate-100">{formatCurrencyBRL(deal.value ?? 0)}</span>
            </div>
            <div className="flex items-center justify-between gap-2">
              <span className="text-slate-500">Tipo</span>
              <span className="text-slate-200">{DEAL_TYPE_LABELS[deal.dealType ?? 'VENDA'] ?? 'Venda'}</span>
            </div>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-200">{probability}%</span>
              </div>
            </div>
            ...
            {priority ? (
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold ring-1 ${
                  PRIORITY_CONFIG[priority]?.color ?? 'bg-white/10 text-slate-300 ring-white/10'
                }`}>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-slate-200">{deal.owner?.name ?? '—'}</span>
            </div>
            {estimatedCommission ? (
              <div className="flex items-center justify-between gap-2">
                <span className="text-slate-500">Comissão</span>
                <span className="font-semibold text-emerald-300" title={`Taxa: ${estimatedCommission.rate}%`}>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-slate-200">{formatAtISO(deal.createdAt)}</span>
            </div>
            <div className="flex items-center justify-between gap-2">
              <span className="text-slate-500">Última mudança</span>
              <span className="text-slate-200">{formatAtISO(deal.updatedAt)}</span>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-1.5 text-xs">
              {deal.items.map((item: any, idx: number) => (
                <div key={item.id ?? idx} className="flex items-center justify-between gap-2">
                  <span className="text-slate-200 truncate">{item.name ?? 'Produto'}</span>
            ...
            <div className="space-y-1.5 text-xs">
              {filledCustomFields.map((def) => (
                <div key={def.id} className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-slate-200 text-right truncate max-w-[60%]">{String(customFields[def.key])}</span>
                </div>
              ))}
            </div>
          </div>
