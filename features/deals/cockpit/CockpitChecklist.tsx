...
export function CockpitChecklist({ checklist, onPersistChecklist, onReload }: CockpitChecklistProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="text-1xs text-muted-foreground">Checklist persistido por deal (salvo em metadata).</div>
        <Button
          type="button"
          className="rounded-lg border border-border bg-background dark:bg-white/2 px-2.5 py-1.5 text-1xs font-semibold text-secondary-foreground dark:text-muted-foreground hover:bg-muted dark:hover:bg-white/5"
          onClick={onReload}
 ...
 ?'mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-md bg-emerald-500 text-foreground'
      ...
      <div className="mt-2 text-1xs text-muted-foreground dark:text-secondary-foreground">Dica: isso fica no deal atual e aparece igual quando você trocar de deal.</div>
    </Panel>
  );
}
