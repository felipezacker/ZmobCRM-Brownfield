          ...
          .map((key) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed line-clamp-3 flex-1">{t.description}</p>
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-white/5 flex gap-2 shrink-0 overflow-hidden">
                  {t.tags.slice(0, 2).map((tag, i) => (
                    <span key={`${key}-tag-${i}`} className="px-2 py-1 rounded-md bg-slate-50 dark:bg-white/5 text-[10px] font-medium text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-white/5 whitespace-nowrap">
                      #{tag}
                    </span>
                  ))}
                </div>
...
export function CommunityList({
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Templates da Comunidade</h3>
      {isLoadingRegistry ? (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <h4 className="font-semibold text-slate-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 flex items-center gap-2">
                  {tpl.name}
                  <span className="text-xs bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded-full text-slate-500">v{tpl.version}</span>
                </h4>
                {isInstalling && <Loader2 className="animate-spin text-primary-500" size={16} />}
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">{tpl.description}</p>
              <div className="flex gap-2 flex-wrap">
                {tpl.tags.map((tag, i) => (
                  <span key={`${tpl.id}-tag-${i}`} className="px-2 py-1 rounded-md bg-white dark:bg-black/20 text-xs text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/5">#{tag}</span>
                ))}
                <span className="text-xs text-slate-400 ml-auto">por {tpl.author}</span>
              </div>
            </Button>
          ))}
        </div>
