    ...
    return (
        <div className="p-4 border-b border-dark-border">
            <div className="flex items-center justify-between mb-2">
                <span className="text-2xs text-secondary-foreground uppercase tracking-wider font-semibold">Negocio</span>
                <span className={`text-2xs px-1.5 py-0.5 rounded ${deal.priority === 'high' ? 'bg-red-500/10 text-red-400' :
            ...
            {contact?.tags && contact.tags.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                    {contact.tags.slice(0, 4).map((tag: string, i: number) => (
                        <span key={i} className="text-2xs px-1.5 py-0.5 bg-primary-500/10 text-primary-400 rounded">
                            #{tag}
                        </span>
                    ))}
                    {contact.tags.length > 4 && (
                        <span className="text-2xs text-muted-foreground">+{contact.tags.length - 4}</span>
                    )}
                </div>
            )}
