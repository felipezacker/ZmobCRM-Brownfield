    ...
    return {
        ...
        listQuickScripts: tool({
            ...
            execute: async ({ category, limit }) => {
                ...
                // RLS handles: system scripts + own scripts
                const { data, error } = await q;
                if (error) return { error: formatSupabaseFailure(error) };
        ...
        generateAndSaveScript: tool({
            description: 'Gera um script de vendas usando IA e salva automaticamente na tabela quick_scripts (nao mais texto solto). Requer aprovacao no card.',
            inputSchema: z.object({
                title: z.string().min(1).describe('Titulo do script'),
                category: z.enum(['followup', 'objection', 'closing', 'intro', 'rescue', 'other']).describe('Categoria'),
                context: z.string().optional().describe('Contexto para gerar o script (deal, contato, situacao)'),
                baseScriptId: z.string().optional().describe('ID de um script existente para usar como base'),
            }),
            needsApproval: !bypassApproval,
            execute: async ({ title, category, context: scriptContext, baseScriptId }) => {
                ...
                if (baseScriptId) {
                    const { data: baseScript } = await supabase
                        ...
                        .eq('id', baseScriptId)
                        .maybeSingle();
                    baseTemplate = baseScript?.template || '';
                }

                // The script content will be generated by the LLM as part of the tool chain.
                // Here we persist the generated script. The LLM should call this tool with
                // the generated template text already composed.
