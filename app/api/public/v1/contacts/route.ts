...
export const GET = withRateLimit(async function GET(request: Request) {
  ...
  let query = sb
    .from('contacts')
    .select('id,name,email,phone,avatar,notes,status,stage,source,birth_date,last_interaction,last_purchase_date,total_value,created_at,updated_at', { count: 'exact' })
