...
export const GET = withRateLimit(async function GET(request: Request, ctx: { params: Promise<{ companyId: string }> }) {
...
export const PATCH = withRateLimit(async function PATCH(request: Request, ctx: { params: Promise<{ companyId: string }> }) {
  const auth = await authPublicApi(request);
  if (!auth.ok) return NextResponse.json(auth.body, { status: auth.status });
