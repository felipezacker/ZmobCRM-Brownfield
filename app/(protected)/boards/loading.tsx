    ...
    <div className="flex h-full flex-col gap-4 p-4 animate-pulse">
      ...
      <div className="flex flex-1 gap-4 overflow-hidden">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex w-72 shrink-0 flex-col gap-3 rounded-2xl border border-white/10 bg-white/3 p-3">
            {/* Column header */}
            <div className="flex items-center justify-between">
            ...
            {Array.from({ length: 3 - Math.floor(i / 2) }).map((_, j) => (
              <div key={j} className="rounded-xl border border-white/10 bg-white/3 p-3 space-y-2">
