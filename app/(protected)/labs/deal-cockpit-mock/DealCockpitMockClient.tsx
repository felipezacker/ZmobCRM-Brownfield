...
function Chip({ children, tone = 'neutral' }: { children: React.ReactNode; tone?: 'neutral' | 'success' | 'danger' }) {
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-1xs font-semibold ${cls}`}>{children}</span>;
}
...
export default function DealCockpitMockClient() {
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <div className="text-sm font-semibold text-muted-foreground">{mock.deal.title}</div>
                <div className="text-xs text-muted-foreground">|</div>
                <div className="truncate text-xs text-muted-foreground">{mock.deal.company}</div>
              </div>
              <div className="mt-1 text-1xs text-secondary-foreground">2. Pipeline de Vendas</div>
            </div>

            <div className="shrink-0 text-right">
              <div className="text-sm font-semibold text-emerald-300">{formatCurrencyBRL(mock.deal.valueBRL)}</div>
              <div className="mt-0.5 text-1xs text-muted-foreground">
            ...
            {stages.map((s, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`mt-1 text-1xs ${isActive ? 'text-muted-foreground' : 'text-muted-foreground'}`}>{s.label}</div>
                </Button>
              );
            })}
          </div>

          <div className="ml-8 hidden text-1xs text-secondary-foreground xl:block">Clique nas etapas para simular movimentação (mock)</div>
        </div>
      </div>
        ...
        <div className="grid h-full min-h-0 gap-4 lg:grid-cols-[360px_1fr_420px] lg:items-stretch">
          ...
          <div className="flex min-h-0 flex-col gap-4 overflow-auto pr-1">
            ...
            >
              ...
              <div className="mt-2 text-1xs text-muted-foreground">Indicador sintético (mock) — objetivo é te dar leitura rápida.</div>
            </Panel>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className={leftDataTab ==='contato'
                        ? 'rounded-full bg-white/8 px-2.5 py-1 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10'
                        : 'rounded-full bg-white/3 px-2.5 py-1 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10 hover:bg-white/5'
                    }
            ...
            >
              {leftDataTab === 'contato' ? (
                <div className="flex min-h-0 flex-col gap-3">
                  <div>
                    <div className="text-sm font-semibold text-muted-foreground">{mock.contact.name}</div>
                    <div className="mt-1 text-xs text-muted-foreground">{mock.contact.role}</div>
                    <div className="mt-3 space-y-2 text-xs">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-muted-foreground">Tel</span>
                        <span className="flex items-center gap-2">
                          <span className="font-mono text-muted-foreground">{mock.contact.phoneE164}</span>
                          <Button
                            type="button"
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="flex items-center gap-2 min-w-0">
                          <span className="truncate text-muted-foreground">{mock.contact.email}</span>
                          <Button
                            type="button"
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-muted-foreground">{mock.contact.source}</span>
                      </div>
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-muted-foreground">Status</span>
                        <span className="text-muted-foreground">{mock.contact.status}</span>
                      </div>
                    </div>
                  </div>

                  <div className="rounded-xl border border-white/10 bg-white/2 p-3">
                    <div className="text-1xs font-semibold uppercase tracking-wide text-muted-foreground">Sinais</div>
                    <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-muted-foreground">Último evento</span>
                        <span className="truncate text-muted-foreground">{latestNonSystem ? `${latestNonSystem.title}${latestNonSystem.subtitle ? ` — ${latestNonSystem.subtitle}` : ''}` : '—'}</span>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex min-h-0 flex-col gap-3">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="rounded-xl border border-white/10 bg-white/2 p-3">
                      ...
                      <div className="mt-1 text-sm font-semibold text-muted-foreground">{mock.deal.probabilityPct}%</div>
                    </div>
                    <div className="rounded-xl border border-white/10 bg-white/2 p-3">
                      <div className="text-muted-foreground">Ativ.</div>
                      <div className="mt-1 text-sm font-semibold text-muted-foreground">10</div>
                    </div>
                    <div className="rounded-xl border border-white/10 bg-white/2 p-3">
                      <div className="text-muted-foreground">Criado</div>
                      <div className="mt-1 text-sm font-semibold text-muted-foreground">{mock.deal.createdAt}</div>
                    </div>
                    <div className="rounded-xl border border-white/10 bg-white/2 p-3">
                      <div className="text-muted-foreground">Atualizado</div>
                      <div className="mt-1 text-sm font-semibold text-muted-foreground">{mock.deal.updatedAt}</div>
                    </div>
                  </div>

                  <div className="rounded-xl border border-white/10 bg-white/2 p-3">
                    <div className="text-1xs font-semibold uppercase tracking-wide text-muted-foreground">Resumo</div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-lg border border-white/10 bg-white/2 p-2">
                        <div className="text-muted-foreground">Valor</div>
                        <div className="mt-0.5 font-semibold text-muted-foreground">{formatCurrencyBRL(mock.deal.valueBRL)}</div>
                      </div>
                      <div className="rounded-lg border border-white/10 bg-white/2 p-2">
                        <div className="text-muted-foreground">Prioridade</div>
                        <div className="mt-0.5 font-semibold text-muted-foreground">{mock.deal.priority}</div>
                      </div>
                      <div className="rounded-lg border border-white/10 bg-white/2 p-2">
                        <div className="text-muted-foreground">Dono</div>
                        <div className="mt-0.5 font-semibold text-muted-foreground">{mock.deal.owner.name}</div>
                      </div>
                      <div className="rounded-lg border border-white/10 bg-white/2 p-2">
                        <div className="text-muted-foreground">Última mudança</div>
                        <div className="mt-0.5 truncate font-semibold text-muted-foreground">{latestMove ? latestMove.at : mock.deal.updatedAt}</div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <Button
                    ...
                    className={kindFilter ==='all'
                        ? 'rounded-full bg-white/8 px-3 py-1.5 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10'
                        : 'rounded-full bg-white/3 px-3 py-1.5 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10 hover:bg-white/5'
                    }
 ...
 ?'rounded-full bg-cyan-500/15 px-3 py-1.5 text-1xs font-semibold text-cyan-100 ring-1 ring-cyan-500/20'
                          : 'rounded-full bg-white/3 px-3 py-1.5 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10 hover:bg-white/5'
                      }
 ...
 ?'rounded-full bg-amber-500/15 px-3 py-1.5 text-1xs font-semibold text-amber-100 ring-1 ring-amber-500/20'
                        : 'rounded-full bg-white/3 px-3 py-1.5 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10 hover:bg-white/5'
                    }
            ...
            <div className="flex min-h-0 flex-1 flex-col rounded-2xl border border-white/10 bg-white/3">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          {t.title !== 'Moveu para' && t.subtitle ? (
                            <div className="mt-0.5 text-1xs text-muted-foreground">{t.subtitle}</div>
                          ) : null}
                        </div>
                        <div className="shrink-0 text-1xs text-muted-foreground">{t.at}</div>
                      </div>
                    </div>
                  ))}
              </div>
              ...
              <div className="border-t border-white/10 px-4 py-3">
                <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                  <span
                    className="text-1xs font-semibold uppercase tracking-wide text-secondary-foreground"
            ...
            <div className="grid min-h-0 gap-4 lg:grid-cols-2 lg:max-h-[30dvh]">
              <div className="flex min-h-0 flex-col rounded-2xl border border-white/10 bg-white/3 p-4">
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  <div className="text-1xs text-muted-foreground">Dica: salve highlights curtos; a timeline vira seu log.</div>
                  <Button
                    type="button"
 ...
 ?'w-full rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-left text-sm text-emerald-100 hover:bg-emerald-500/15'
                            : 'w-full rounded-xl border border-white/10 bg-white/2 px-3 py-2 text-left text-sm text-muted-foreground hover:bg-white/3'
                        }
                        onClick={() => {
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {item.hint ? <div className="mt-0.5 text-1xs text-muted-foreground">{item.hint}</div> : null}
                          </div>
                          <div className="shrink-0">
                            <span
                              className={checked
 ?'inline-flex items-center rounded-full bg-emerald-500/15 px-2.5 py-1 text-1xs font-semibold text-emerald-100 ring-1 ring-emerald-500/20'
                                  : 'inline-flex items-center rounded-full bg-white/5 px-2.5 py-1 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10'
                              }
                            >
              ...
              <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-1xs text-muted-foreground">Deal: {mock.deal.title}</div>
                  </div>
                </div>
              ...
              <div className="min-h-0 flex-1 overflow-hidden p-4">
                {tab === 'chat' ? (
                  <div className="flex h-full min-h-0 flex-col gap-3">
                    <div className="flex min-h-0 flex-1 flex-col rounded-2xl border border-white/10 bg-white/2">
                      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                        ...
                        <span className="text-1xs text-muted-foreground">Sem chamadas externas</span>
                      </div>
                          ...
                          {chatMessages.map((m) => (
                            <div
                              ...
                              className={m.role ==='user'
                                  ...
                                  : m.role === 'system'
                                    ? 'mx-auto w-fit max-w-[90%] rounded-full bg-white/5 px-3 py-1.5 text-1xs text-muted-foreground ring-1 ring-white/10'
                            ...
                            >
                              ...
                              {m.role === 'assistant' && m.text.startsWith('Mensagem WhatsApp (mock):') ? (
                                <div className="mt-2 flex flex-wrap gap-2">
                                  <Button
                                    type="button"
                                    className="inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-1.5 text-1xs font-semibold text-muted-foreground ring-1 ring-white/10 hover:bg-white/10"
                                    onClick={() => {
                                  ...
                                  <Button
                                    type="button"
                                    className="inline-flex items-center gap-2 rounded-full bg-emerald-500/15 px-3 py-1.5 text-1xs font-semibold text-emerald-200 ring-1 ring-emerald-500/20 hover:bg-emerald-500/20"
                                    onClick={() => {
                                      const msg = lastGeneratedWhatsAppMessage ?? buildWhatsAppMessage();
                                      openWhatsApp(msg);
                      ...
                      {mock.quickActions.map((a) => (
                        <Button
                          ...
                          className={a.tone ==='success'
                              ...
                              : 'inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-2 text-xs font-semibold text-muted-foreground ring-1 ring-white/10 hover:bg-white/10'
                          }
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/2 p-4">
                    ...
                    {savedNotes.length === 0 ? (
                      <div className="mt-3 text-sm text-muted-foreground">Sem notas ainda. Salve uma nota no centro para ela aparecer aqui.</div>
                    ) : (
                      <div className="mt-3 space-y-2">
                        {savedNotes.map((n) => (
                          <div key={n.id} className="rounded-2xl border border-white/10 bg-white/3 p-3">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                ...
                                <div className="mt-2 text-1xs text-muted-foreground">{n.at}</div>
                              </div>
                              <div className="shrink-0">
                                <Button
                                  type="button"
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/2 p-4">
                    ...
                    <div className="mt-3 text-sm text-muted-foreground">(mock) Argumentos, objeções, playbooks por etapa.</div>
                  </div>
                ) : (
                  <div className="rounded-2xl border border-white/10 bg-white/2 p-4">
                    <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
                    ...
                    <div className="mt-3 text-sm text-muted-foreground">(mock) Propostas, PDFs, contratos, anexos.</div>
                  </div>
                )}
              </div>
            </div>
